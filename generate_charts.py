...
# Based on https://github.com/tortoise/orm-benchmarks/blob/6a5b4ebd435b6310752326afe0e80eb3de339584/generate_charts.py
...
def parse_results(filepath):
    ...
    with open(filepath) as f:
        for line in f:
            m = re.match(r"^\s*(?:\d+[→\t])?\s*(.+?),\s*([A-G]):\s*Rows/sec:\s*([\d.]+)", line)
            if m:
...
def generate_chart(data, title, output_path, show_geomean=True):
    ...
    fig, ax = plt.subplots(figsize=(max(14, int(n_ops * 1.5)), 7))
